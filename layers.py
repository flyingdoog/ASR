...
class Dense(Layer):
	...
	def __init__(self, input_dim, output_dim, dropout=0., sparse_inputs=False,
		...
		if kernel_initializer==get_glorot:
			with tf.variable_scope(self.name + '_vars',reuse=tf.AUTO_REUSE):
				self.vars['weights'] = tf.get_variable(name='weights',shape =[input_dim, output_dim],  dtype=tf.float32)
				self.vars['bias'] = tf.get_variable(name='bias',shape=[output_dim],dtype=tf.float32)
